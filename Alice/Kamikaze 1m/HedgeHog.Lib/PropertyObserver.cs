...
namespace MvvmFoundation.Wpf
{
      ...
      where TPropertySource : INotifyPropertyChanged
  {
    ...
    public PropertyObserver(TPropertySource propertySource)
    {
      ...
      _propertyNameToNameHandlerMap = new Dictionary<string, Action<TPropertySource, string>>();
    }
        ...
        Action<TPropertySource> handler)
    {
      ...
      if (propertySource != null)
      {
        ...
        Debug.Assert(!_propertyNameToNameHandlerMap.ContainsKey(propertyName), "Why is the '" + propertyName + "' property being registered again?");
    ...
    public PropertyObserver<TPropertySource> RegisterHandler(
        Expression<Func<TPropertySource, object>> expression,
        Action<TPropertySource, string> handler)
    {
      if (expression == null)
        throw new ArgumentNullException("expression");
      ...
      if (propertySource != null)
      {
        ...
        _propertyNameToNameHandlerMap[propertyName] = handler;
        PropertyChangedEventManager.AddListener(propertySource, this, propertyName);
      }
    ...
    public PropertyObserver<TPropertySource> UnregisterHandler(Expression<Func<TPropertySource, object>> expression)
    {
      ...
      if (propertySource != null)
      {
        ...
        else if (_propertyNameToNameHandlerMap.ContainsKey(propertyName))
        {
          _propertyNameToNameHandlerMap.Remove(propertyName);
          PropertyChangedEventManager.RemoveListener(propertySource, this, propertyName);
        }
      }
      ...
      if (managerType == typeof(PropertyChangedEventManager))
      {
        ...
        if (args != null && sender is TPropertySource)
        {
          ...
          if (String.IsNullOrEmpty(propertyName))
          {
            ...
            foreach (Action<TPropertySource, string> nameHandler in _propertyNameToNameHandlerMap.Values.ToArray())
              nameHandler(propertySource, string.Empty);
          ...
          {
            Action<TPropertySource> handler;
            Action<TPropertySource, string> nameHandler;
            ...
            else if (_propertyNameToNameHandlerMap.TryGetValue(propertyName, out nameHandler))
            {
              nameHandler(propertySource, propertyName);
    ...
    readonly Dictionary<string, Action<TPropertySource, string>> _propertyNameToNameHandlerMap;
