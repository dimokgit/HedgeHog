  ...
  static class EdgeTests {
    public static void MoveEdge(AccountManager am) {
      ...
      var parameter = (
        ...
        .Do(p => Program.HandleMessage(new { p.edge, p.param.contract, p.param.limitPrice, replaceOrders = p.param.replaceOrders.Flatter(",") }.ToTextTable(), false))
      ...
      (from par in parameter
       ...
       select och
       ).Subscribe(ochs =>Program.HandleMessage(ochs.Select(och=>new {och.holder,och.error }).ToTextTable()));
