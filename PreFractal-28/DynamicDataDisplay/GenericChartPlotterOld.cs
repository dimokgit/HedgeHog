...
{
	[Obsolete("Will be revised or removed.")]
	public class GenericChartPlotterOld<THorizontal, TVertical> : ChartPlotter
	{
		public GenericChartPlotterOld()
		{
			if (this is GenericChartPlotterOld<double, TVertical>)
			{
				var plotter = this as GenericChartPlotterOld<double, TVertical>;
				plotter.HorizontalToDouble = x => x;
				plotter.DoubleToHorizontal = x => x;
				plotter.HorizontalAxis = new HorizontalAxis();
			}
			else if (this is GenericChartPlotterOld<TimeSpan, TVertical>)
			{
				var plotter = this as GenericChartPlotterOld<TimeSpan, TVertical>;
			...
			else if (this is GenericChartPlotterOld<DateTime, TVertical>)
			{
				var plotter = this as GenericChartPlotterOld<DateTime, TVertical>;
			...
			if (this is GenericChartPlotterOld<THorizontal, double>)
			{
				var plotter = this as GenericChartPlotterOld<THorizontal, double>;
				plotter.VerticalToDouble = x => x;
				plotter.DoubleToVertical = x => x;

				plotter.VerticalAxis = new VerticalAxis();
			}
			else if (this is GenericChartPlotterOld<THorizontal, TimeSpan>)
			{
				var plotter = this as GenericChartPlotterOld<THorizontal, TimeSpan>;
			...
			else if (this is GenericChartPlotterOld<THorizontal, DateTime>)
			{
				var plotter = this as GenericChartPlotterOld<THorizontal, DateTime>;
		...
		protected override void ValidateVerticalAxis(GeneralAxis axis)
		{
			if (!(axis is AxisBase<TVertical>))
			{
				throw new ArgumentException(String.Format(CultureInfo.CurrentCulture, Strings.Exceptions.InvalidGenericAxis, GetType().Name, typeof(TVertical).Name));
			}
		...
		{
			get { return (AxisBase<THorizontal>)base.MainHorizontalAxis; }
			set { base.MainHorizontalAxis = value; }
		}

		public new AxisBase<TVertical> VerticalAxis
		{
			get { return (AxisBase<TVertical>)base.MainVerticalAxis; }
			set { base.MainVerticalAxis = value; }
		}
		...
		public GenericRect<THorizontal, TVertical> VisibleRect
		{
			get
			{
				DataRect visible = Viewport.Visible;
				var rect = new GenericRect<THorizontal, TVertical>(
					DoubleToHorizontal(visible.XMin),
					DoubleToVertical(visible.YMin),
					DoubleToHorizontal(visible.XMax),
					DoubleToVertical(visible.YMax));
			...
			set
			{
				Rect visible = new Rect(
					new Point(
						HorizontalToDouble(value.XMin),
						VerticalToDouble(value.YMin)
						),
					new Point(
						HorizontalToDouble(value.XMax),
						VerticalToDouble(value.YMax)));
		...
		private sealed class HorizontalConversionWrapper<TH, TV> : IValueConversion<TH>
		{
			...
			private readonly GenericChartPlotterOld<TH, TV> plotter;
