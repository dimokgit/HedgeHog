...
namespace HedgeHog.Alice.Client {
  ...
  public class RemoteControlModel : RemoteControlModelBase {
    ...
    void CopyTradingMacro(object tradingMacro,string tradingMacroName= null) {
      ...
      try {
        GlobalStorage.UseAliceContext(c => c.TradingMacroes.AddObject(tmNew), true);
    ...
    void UpdateTradingStatistics() {
      if (GetTradingMacros().Any(tm => !tm.RatesArraySafe.Any())) return;
      ...
      _tradingStatistics.TakeProfitPips = tms.Select(tm => tm.TakeProfitPips).ToArray().AverageByIterations(2).Average();
    ...
    void ShowChart(TradingMacro tm) {
      try {
        ...
        timeHigh = tm.CorridorStats.Rates.Last().StartDateContinuous;
        ...
        try {
          ...
          charter.GetPriceHigh = tm.CorridorCrossGetHighPrice();// tm.CorridorStats.priceHigh;
          charter.GetPriceLow = tm.CorridorCrossGetLowPrice();// tm.CorridorStats.priceLow;
    ...
    void fw_OrderAdded(object sender, OrderEventArgs e) {
      Order order = e.Order;
      if (order.IsEntryOrder && !order.IsNetOrder) {
    ...
    void RemoveEntryOrder(string pair) {
      try {
        var fw = tradesManager as FXCoreWrapper;
        if (fw == null) return;
        foreach (var order in tradesManager.GetOrders("").Where(o => o.Pair == pair && o.IsEntryOrder && !o.IsNetOrder))
    ...
    PendingOrder OpenChainTrade(TradingMacro tm, bool isBuy) {
      ...
      var stop = tm.FreezeStopType == Freezing.None ? 0 : GetStopByFractal(0,tm,tm.RatesArraySafe, isBuy, tradesManager.ServerTime);
