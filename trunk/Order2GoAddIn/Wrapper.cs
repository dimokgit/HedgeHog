...
namespace Order2GoAddIn {
  ...
  public class FXCoreWrapper:IDisposable,ITradesManager {
    ...
    void OnPriceChanged(Price price,Account account,Trade[] trades) {
      if (PriceChangedEvent != null) PriceChangedEvent(this, new PriceChangedEventArgs(price,account, trades));
    }
    #endregion
    ...
    public bool Hedging {
      get {
        if (_Hedging == null) _Hedging = GetAccount().Hedging;
    ...
    public Account GetAccount() {
      if (accountInternal == null) {
        accountInternal = GetAccount_Slow(true);
        SetTrades();
      }
    ...
    private Account GetAccount(FXCore.ParserAut row) {
      ...
      accountInternal.StopAmount = trades.Sum(t => t.StopAmount);
      accountInternal.LimitAmount = trades.Sum(t => t.LimitAmount);
      accountInternal.ServerTime = ServerTime;
      accountInternal.PipsToMC = (int)(accountInternal.UsableMargin * PipsToMarginCallPerUnitCurrency(trades));
      return accountInternal;
    }
    ...
    public Order[] GetOrders(string Pair,bool getFromInternal = false) {
      return (from order in (getFromInternal ? GetOrdersInternal(Pair) : EntryOrders.Values.ToArray())
    ...
    public Order[] GetOrdersInternal(string Pair) {
      ...
      try {
        ...
        return orderList.OrderBy(_ordersOrderBy).ToArray();
    ...
    Order InitOrder_(FXCore.ParserAut row) {
      var order = new Order();
      order.OrderID = (String)row.GetValue("OrderID");
      order.RequestID = (String)row.GetValue("RequestID");
      order.AccountID = (String)row.GetValue("AccountID");
    ...
    public FXCore.TablesManagerAut TablesManager {
      get {
        if (_tablesManager == null && IsLoggedIn)
          _tablesManager = (FXCore.TablesManagerAut)Desk.TablesManager;
        return IsLoggedIn ? _tablesManager : null;
      }
    }
//    Dictionary<string, int> tableIndices = new Dictionary<string, int>();
    Dictionary<string, FXCore.TableAut> tableIndices = new Dictionary<string, FXCore.TableAut>();
    [CLSCompliant(false)]
    ...
    public string CreateEntryOrder(string pair, bool isBuy, int amount, double rate, double stop, double limit) {
      ...
      Desk.CreateEntryOrder2(AccountID, pair, isBuy, amount, rate,  slType , stop, limit, 0, out psOrderId, out psDI);
      if (!string.IsNullOrWhiteSpace(psOrderId+""))
        try {
          Order newOrder = null;
          while (newOrder == null) {
            Thread.Sleep(0);
            newOrder = GetOrders(Pair, true).Where(o => o.OrderID == psOrderId + "").SingleOrDefault();
          }
          _entryOrders[psOrderId + ""] = newOrder;
        } catch (Exception exc) {
          throw;
        }
      return psOrderId + "";
      ...
      try {
        if (this.IsFIFO(pair)) {
          ...
          while (lot > 0) {
            ...
            if (tradeToClose == null) return false;
    ...
    public void FixOrderSetLimit(string tradeId, double takeProfit, string remark) {
      FixOrderSetLimit(tradeId, takeProfit, remark, true);
    }
    [MethodImpl(MethodImplOptions.Synchronized)]
    public void FixOrderSetLimit(string tradeId, double takeProfit, string remark, bool addToCurrent) {
      try {
        object a = "", b;
        ...
        if (IsFIFO(trade.Pair)) {
          if (takeProfit == 0)
            ...
            if (order == null || Round(pair, order.Rate, -1) != Round(pair, limitRate, -1))
              try {
                Desk.ChangeNetStopLimit(pair, AccountID, isBuy, limitRate, false, out a);
                Debug.WriteLine("ChangeNetLimit for " + pair);
              } catch (Exception exc) {
                RaiseError(new ErrorEventArgs(exc));
              }
          }
    ...
    public void FixOrderSetStop(string tradeId, double stopLoss, string remark) {
      FixOrderSetStop(tradeId, stopLoss, remark, true);
    }
    /// <summary>
    /// 
    /// </summary>
    ...
    [MethodImpl(MethodImplOptions.Synchronized)]
    public void FixOrderSetStop(string tradeId, double stopLoss, string remark, bool addToCurrent) {
      object a = "", b;
      var trade = GetTrade(tradeId);
      var isBuy = trade.Buy;
      var pair = trade.Pair;
      ...
      if (IsFIFO(trade.Pair)) {
        if (stopLoss == 0)
          ...
          if (addToCurrent && order != null)
            stopRate = (trade.Buy ? order.SellRate : order.BuyRate) + stopLoss;
          else {
            var trades = GetTradesInternal(pair);
            stopRate = trades.NetOpen() + spreadToAdd + stopLoss;
          }
          if (order == null || Round(pair, order.Rate, -1) != Round(pair, stopRate, -1)) {
            ...
            if( isBuy && stopRate < trades.Min(t=>t.Close) || !isBuy && stopRate > trades.Max(t=>t.Close))
              try {
                Desk.ChangeNetStopLimit(pair, AccountID, isBuy, stopRate, true, out a);
                Debug.WriteLine("ChangeNetStop for " + pair);
              } catch (Exception exc) {
                RaiseError(exc);
              }
          }
    ...
    Dictionary<string, Order> EntryOrders {
      get {
        if (_entryOrders == null)
          _entryOrders = new Dictionary<string, Order>(GetOrdersInternal("").ToDictionary(o => o.OrderID, o => o));
        return _entryOrders;
      }
    }

    Dictionary<string, Trade> OpenTrades = new Dictionary<string, Trade>();
    ...
    void mSink_ITradeDeskEvents_Event_OnRowAddedEx(object _table, string RowID,string rowText) {
      try {
        ...
        switch (table.Type.ToLower()) {
          #region Trades
          case "trades": {
              var tradeParsed = GetTradeSure(RowID,parser,rowText,table);
              OpenTrades[RowID] = tradeParsed;
    ...
    #region Parser
    FXCore.ParserAut _parser;
    public FXCore.ParserAut Parser {
      get {
        if (_parser == null)
          _parser = Desk.GetParser() as FXCore.ParserAut;
        return _parser;
      }
    }
    #endregion
      ...
      try {
        ...
        switch (table.Type.ToLower()) {
          case "offers":
            ...
            OnPriceChanged(price,GetAccount(),GetTrades());
            break;
          case TABLE_ACCOUNTS:
            //SetTrades();
            Parser.ParseEventRow(rowText, table.Type);
          ...
          case TABLE_ORDERS:
            ...
            break;
          case TABLE_TRADES:
            var trade = GetTradeSure(rowID,Parser,rowText,table);
    ...
    private Trade GetTradeSure(string rowID, FXCore.ParserAut Parser, string rowText, FXCore.TableAut table) {
      Parser.ParseEventRow(rowText, table.Type);
    ...
    void mSink_ITradeDeskEvents_Event_OnRowBeforeRemoveEx(object _table, string RowID, string sExtInfo) {
      try {
        ...
        switch (table.Type.ToLower()) {
          case TABLE_TRADES:
            var trade = GetTradeSure(RowID,parser,sExtInfo,table);
    ...
    DateTime ConvertDateToLocal(DateTime date) {
      lock (converterLocker) {
        return TimeZoneConverter.Convert(date, TimeZoneConverter.ZONE_UTC, TimeZoneConverter.ZONE_LOCAL);
      }
    }

    FXCore.ITimeZoneConverterAut _TimeZoneConverter;
    FXCore.ITimeZoneConverterAut TimeZoneConverter {
      get {
        if (_TimeZoneConverter == null)
          _TimeZoneConverter = coreFX.Desk.TimeZoneConverter as FXCore.ITimeZoneConverterAut; ;
        return _TimeZoneConverter;
      }
    }
    DateTime ConvertDateToUTC(DateTime date) {
      lock (converterLocker) {
        return TimeZoneConverter.Convert(date, TimeZoneConverter.ZONE_LOCAL, TimeZoneConverter.ZONE_UTC);
      }
    }
