...
namespace HedgeHog.Alice.Store {
  ...
  public partial class TradingMacro {
    ...
    public void OnLoadRates(Action a = null) => LoadRatesAsyncBuffer.Push(() => {
      ...
      if(_activeFastRequests > 0) {
        if(!IsFast(this)) return;
        _activeFastRequests--;
      }
      LoadRates(a);
      bool IsFast(TradingMacro tm) => tm.BarPeriod == BarsPeriodType.t1;
    });
    ...
    PriceChangedAsyncBuffer PriceChangedAsyncBufferInstance =>
      (_priceChangedAsyncBuffer == null || _priceChangedAsyncBuffer.Disposed)
...
new MarketPrice(
    ...
    public void Replay(ReplayArguments<TradingMacro> args) {
      ...
      var otherTMs = tms().Where(tm => tm != this).ToList();
...
TradesManagerStatic.PipAmount(Pair, Trades.Lots(), (TradesManager?.RateForPipAmount(CurrentPrice.Ask, CurrentPrice.Bid)).GetValueOrDefault(), PointSize);
        ...
        UseRatesInternal(ri => ri.BackwardsIterator().Select(r => RoundTime(r.StartDate)).Take(1))
          ...
          .ForEach(lastRateDate => {
            ...
            if(priceDate > lastRateDate) {
              UseRatesInternal(ri => ri.Add(isTick ? new Tick(price, 0, false) : new Rate(price, false)));
              if(BarPeriod > 0) {
                Log = new DebugLogException(new { OnLoadRates = $"Forced - {this}" } + "");
                OnLoadRates();
              } else
      ...
      try {
        TradingRatioByPMC.Yield()
          ...
          .Concat(TradingRatio.Yield()
            ...
            .Select(tr =>
            tr >= 1
            ? (buy: (tr * BaseUnitSize / BaseUnitSize).ToInt(), sell: (tr * BaseUnitSize / BaseUnitSize).ToInt())
    ...
    public IEnumerable<T> UseStraddleHistory<T>(Func<TradingMacro, List<MarketPrice>> history, Func<List<MarketPrice>, T> func, int timeoutInMilliseconds = 500, [CallerMemberName] string Caller = "") {
      ...
      Stopwatch sw = Stopwatch.StartNew();
      try {
        return TradingMacroTrader(tm => func(history(tm)));
      } catch(Exception exc) {
        Log = exc;
        return new T[0];
      } finally {
            ...
            UseRatesInternal(ri => ri.BackwardsIterator().TakeWhile(isNotHistory).ToList()).ForEach(ratesLocal => {
              ...
              if(ratesList.Count > 0) {
                ...
                UseRatesInternal(rl => {
                  ...
                  if(rl.Count < BarsCount) {
                    Log = new Exception($"Rates count {rl.Count} < {new { BarsCount }}");
                    TriggerRatesClear = true;
                  }
                  return;
                });
              } else
    ...
    void LoadRatesImpl(ITradesManager fw, string pair, int periodMinutes, int periodsBack, DateTime startDate, DateTime endDate, bool? isFast, bool? useRTH, List<Rate> ratesList, bool groupToSeconds) {
      ...
      //}
      fw.GetBars(pair, periodMinutes, periodsBack, startDate, endDate, isFast, useRTH, ratesList, z => {
        if(groupToSeconds)
    ...
    class LoadRateAsyncBuffer :AsyncBuffer<LoadRateAsyncBuffer, Action> {
      public LoadRateAsyncBuffer(int tmCount) : base(1, 22.FromSeconds().Multiply(tmCount), true, (Func<string>)null) { }
