CREATE TABLE [dbo].[t_Bar] (
    ...
    [Row]            INT                NOT NULL,
    [StartDateEst]   AS                 (CONVERT([datetime2],([StartDate] AT TIME ZONE 'US Eastern Standard Time'))),
    CONSTRAINT [PK_t_Bar] PRIMARY KEY NONCLUSTERED ([ID] ASC) WITH (IGNORE_DUP_KEY = ON)
);


GO
CREATE UNIQUE CLUSTERED INDEX [IX_t_Bar]
    ON [dbo].[t_Bar]([Pair] ASC, [Period] ASC, [StartDate] ASC) WITH (IGNORE_DUP_KEY = ON);


GO
CREATE NONCLUSTERED INDEX [IX_t_Bar_PairPeriod]
    ON [dbo].[t_Bar]([Pair] ASC, [Period] ASC);
