CREATE PROCEDURE [dbo].[sRunPunchPairs] AS
...
FROM vPunchActive PIn 
INNER JOIN vPunchActive POut
	ON PIn.DirectionId = @DirIn AND POut.Time > PIn.Time 	AND POut.DirectionId = @DirOut
LEFT OUTER JOIN vPunchActive PNextIn ON PNextIn.Time > POut.Time 	AND PNextIn.DirectionId = @DirIn
LEFT OUTER JOIN vPunchActive PNextOut ON PNextOut.Time > PNextIn.Time AND PNextOut.DirectionId = @DirOut
...
SELECT TOP 1 PIn.Time TimeIn,POut.Time TimeOut,PNextIn.Time TimeNextIn,PNextOut.Time TimeNextOut
FROM vPunchActive PIn 
INNER JOIN vPunchActive POut ON PIn.DirectionId = @DirIn AND POut.Time > PIn.Time AND POut.DirectionId = @DirOut
LEFT OUTER JOIN vPunchActive PNextIn ON PNextIn.Time > POut.Time AND PNextIn.DirectionId = @DirIn
LEFT OUTER JOIN vPunchActive PNextOut ON PNextOut.Time > PNextIn.Time AND PNextOut.DirectionId = @DirOut
